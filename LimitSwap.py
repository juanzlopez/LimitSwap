...
def check_approval(token, address, allowancetocomparewith):
    ...
    contract = client.eth.contract(address=Web3.toChecksumAddress(address), abi=standardAbi)
    actualallowance = contract.functions.allowance(Web3.toChecksumAddress(settings['WALLETADDRESS']),
                                                   routerAddress).call()
                ...
                if token['ENABLED'] == 'true':
                    ...
                    #
                        ...
                        if quote > Decimal(token['SELLPRICEINBASE']) or quote < Decimal(token['STOPLOSSPRICEINBASE']):
                            ...
                            printt_info("We have bought and sold a position in", token['SYMBOL'],
